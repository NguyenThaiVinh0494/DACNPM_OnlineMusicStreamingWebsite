...
export default function AddToPlaylistModal() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative bg-[#2d2f32] border border-white/10 w-full max-w-md rounded-2xl shadow-2xl p-6 flex flex-col max-h-[80vh]">
        <button 
          onClick={closeAddToPlaylistModal}
          className="absolute top-4 right-4 p-2 text-nct-text-dim hover:text-white rounded-full hover:bg-white/10 transition-colors"
        >
          <FiX className="w-5 h-5" />
        </button>

        <h3 className="text-xl font-bold text-white mb-6">Add to playlists</h3>

        <div className="overflow-y-auto flex-1 pr-2 space-y-2 custom-scrollbar">
          {myPlaylists.length === 0 ? (
            <div className="py-8 text-center text-nct-text-dim flex flex-col items-center">
            ...
            myPlaylists.map(playlist => {
              ...
              return (
                <div key={playlist.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-white/5 transition-colors group">
                  <div className="w-12 h-12 rounded-lg overflow-hidden shrink-0 bg-white/10 flex items-center justify-center">
                    {playlist.image ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-nct-text-dim">{playlist.songs.length} songs</p>
                  </div>
                  <button
                    disabled={isAdded}
                    ...
                    className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors shrink-0 border ${
                      isAdded 
                        ...
                        : 'bg-transparent border-white/30 text-white hover:border-nct-primary hover:text-nct-primary'
                    }`}
                  >
                    {isAdded ? 'Added' : 'Add'}
                  </button>
