  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-[#2d2f32] border border-gray-200 dark:border-white/10 w-full max-w-md rounded-2xl shadow-2xl p-6 flex flex-col max-h-[80vh]">
        <button 
          onClick={closeAddToPlaylistModal}
          className="absolute top-4 right-4 p-2 text-gray-500 hover:text-gray-900 dark:text-nct-text-dim dark:hover:text-white rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors"
        >
          <FiX className="w-5 h-5" />
        </button>

        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">{t('add_to_playlists')}</h3>
        ...
        <div className="overflow-y-auto flex-1 pr-2 space-y-2 custom-scrollbar">
          {myPlaylists.length === 0 ? (
            <div className="py-8 text-center text-gray-500 dark:text-nct-text-dim flex flex-col items-center">
              ...
              <p>{t('no_playlists_yet')}</p>
              <p className="text-sm mt-1">{t('create_from_sidebar')}</p>
            </div>
          ) : (
              ...
              return (
                <div key={playlist.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-500 dark:text-nct-text-dim">{playlist.songs.length} {t('songs')}</p>
                  </div>
                  <button
                    disabled={isAdded}
