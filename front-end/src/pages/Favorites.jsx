    ...
    <div className="pb-20">
      <div className="flex flex-col gap-8">
        ...
        <div className="flex items-start gap-8">
          <div className="w-[240px] h-[240px] shadow-2xl rounded-xl overflow-hidden group relative bg-[#2a2a2a] flex items-center justify-center">
            {favorites.length > 0 ? (
              <>
                <img 
              ...
              <FiMusic className="w-20 h-20 text-white/10" />
            )}
          </div>
        ...
        <div className="bg-transparent mt-4">
            ...
            <div className="flex items-center text-white text-sm font-medium px-4 py-0 border-b border-white/5 bg-[#282828] h-[56px] rounded-t-md">
              ...
              <div className="flex-1 flex items-center gap-5 pl-2">
                ...
                <div className="flex items-center gap-5 border-l border-white/10 pl-5">
                  <button 
                    className="flex items-center gap-2 text-white hover:text-nct-primary transition-colors"
                    onClick={() => {
                  ...
                  <button className="flex items-center gap-2 text-white hover:text-nct-primary transition-colors">
          ...
          <div className="flex flex-col mt-2">
            {favorites.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-[#b3b3b3] border-t border-white/5 mt-4">
                <FiMusic className="w-16 h-16 mb-4 opacity-20" />
                <p className="text-lg font-medium mb-2">Chưa có bài hát yêu thích nào</p>
                ...
                return (
                  <div key={song.id} className={`flex items-center px-4 py-2 transition-colors group rounded-md relative ${isSelected ? 'bg-[#323232]' : 'hover:bg-[#2b2b2b]'}`}>
                    ...
                    <div className={`absolute right-4 flex items-center justify-end gap-2 pr-4 transition-opacity dropdown-container ${openDropdown === song.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                      ...
                      >
                        <FiHeart className="w-4 h-4 text-nct-primary fill-[#2daaed] hover:scale-110 transition-transform" />
                      </button>
                      
                      <button 
                        onClick={() => toggleDropdown(song.id)}
                        className={`p-2 rounded-full transition-colors relative ${openDropdown === song.id ? 'bg-nct-primary text-white' : 'hover:bg-white/10 text-[#b3b3b3] hover:text-white'}`}
                        title="Thêm"
                      >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="relative bg-[#1e1e1e] border border-white/10 w-full max-w-4xl h-[75vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-[#2a2a2a]">
              <h3 className="text-xl font-bold text-white">Thêm bài hát vào Yêu thích</h3>
              <button 
                onClick={() => setIsAddModalOpen(false)}
            ...
            <div className="flex-1 flex overflow-hidden">
              <div className="w-1/3 border-r border-white/5 bg-[#121212] p-4">
                <button 
              ...
              <div className="w-2/3 flex flex-col bg-[#1e1e1e]">
                <div className="p-4 border-b border-white/5">
                  <div className="relative">
                    <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-[#b3b3b3] w-5 h-5" />
                    <input 
                      type="text" 
                      placeholder="Tìm kiếm bài hát..."
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="w-full bg-[#2a2a2a] text-white placeholder-[#b3b3b3] rounded-lg py-2.5 pl-10 pr-4 outline-none focus:bg-[#333] transition-colors"
                    />
                  </div>
                </div>
                <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
                  {filteredSongs.length === 0 ? (
                    <div className="text-center py-10 text-[#b3b3b3]">Không tìm thấy bài hát nào.</div>
                  ) : (
                    filteredSongs.map(song => {
                      ...
                      return (
                        <div key={song.id} className="flex items-center gap-4 px-4 py-2.5 hover:bg-white/5 rounded-lg group transition-colors">
                          ...
                          {isAdded ? (
                            <span className="text-[#b3b3b3] text-sm font-medium px-3">Đã yêu thích</span>
                          ) : (
                            <button 
                              onClick={() => toggleFavorite(song)}
